...
class FineTuningRefDataset(Dataset):
    def __init__(self, ref_img, ref_lmark, device):
        self.data = ref_img
        self.lmark = ref_lmark
        self.device = device
    ...
    def __getitem__(self, idx):
        frame_has_face = False
        while not frame_has_face:
            sel_id = np.random.randint(0, len(self.data), 1).item()
            frame_mark = generate_cropped_landmarks([self.data[sel_id]], [self.lmark[sel_id].copy()], pad=50)
            frame_has_face = True
        frame_mark = torch.from_numpy(np.array(frame_mark)).type(dtype = torch.float) #1,2,256,256,3
        frame_mark = frame_mark.transpose(2,4).to(self.device) #1,2,3,256,256
