...
class LossCnt_self(nn.Module):
    ...
    def forward(self, x, x_hat, vgg19_weight=1e-2, vggface_weight=2e-3):
        ...
        #     lossface += l1_loss(x_feat.detach(), xhat_feat)


        """Retrieve vggface feature maps"""
        #define hook
        x_vgg, x_hat_vgg = self.VGG19(x), self.VGG19(x_hat)
        loss19 = 0
        for i in range(len(x_vgg)):
            loss19 += l1_loss(x_vgg[i].detach(), x_hat_vgg[i])        
...
class LossG(nn.Module):
    ...
    def __init__(self, VGGFace_body_path, VGGFace_weight_path, device, vgg19_weight=1e-2, vggface_weight=2e-3):
        ...
        # self.LossCnt = LossCnt_self(VGGFace_body_path, VGGFace_weight_path, device)
...
class LossG_test(nn.Module):
    def __init__(self, VGGFace_body_path, VGGFace_weight_path, device, vgg19_weight=1e-2, vggface_weight=2e-3):
        super(LossG_test, self).__init__()
