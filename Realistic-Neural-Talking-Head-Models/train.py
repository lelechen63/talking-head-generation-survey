...
for epoch in tqdm(range(epochCurrent, num_epochs)):
    for i_batch, (f_lm, x, g_y, i) in enumerate(tqdm(dataLoader), start=i_batch_current):
        ...
        if i_batch % 10 == 0:
            ...
            plt.imshow(out)
            # plt.show()
            plt.savefig('./experiment/temp/generate_{}.png'.format(i_batch))
            plt.close()
            ...
            plt.imshow(out)
            # plt.show()
            plt.savefig('./experiment/temp/gt_{}.png'.format(i_batch))
            plt.close()
            ...
            plt.imshow(out)
            # plt.show()
            plt.savefig('./experiment/temp/landmark_{}.png'.format(i_batch))
            plt.close()
        ...
        if i_batch % 100 == 99:
            ...
            log_value('lossD', lossD.item(), current)
