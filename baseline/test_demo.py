...
pickle_files = np.load('vox_demo.npy')
pickle_data = []
...
for paths in pickle_files:
    ...
    if not os.path.exists(audio_file) or not os.path.exists(landmark_file) or not os.path.exists(rt_file):
        continue
    cur_data = [file.replace('_aligned.mp4', ''), paths[1], paths[2]]
...
save_root = os.path.join('extra_degree_result', save_name, '{}_shot_epoch_{}'.format(opt.n_shot, opt.which_epoch))
