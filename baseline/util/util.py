...
def eye_openrate(lmarks):
    eyes = [[37,41],[38,40] ,[43,47],[44,46]]  # [upper, lower] , [left1,left2, right1, right1]
    openrate = [np.linalg.norm(lmarks[:, eye[0], :2] - lmarks[:, eye[1], :2], axis=1) for eye in eyes]
    # openrate = [np.sum(np.abs(lmarks[:, eye[0], :2] - lmarks[:, eye[1], :2]), axis=1) for eye in eyes]
    openrate = sum(openrate)
    return openrate
